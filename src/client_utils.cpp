 ...
 * function calls to non-async-safe functions, or anything that allocates
 * memory.
 */
 ...
 */
static void print_chat_message(std::string_view sender,
...
/**
 * Handles prompting user input and sending a request for a world message to the
 * server.
 *
 ...
 */
static int handle_client_registration(conn_t &conn, std::string &username,
                                      std::string &password) {
 ...
 */
static int handle_client_login(conn_t &conn, std::string &username,
                               std::string &password) {
...
/**
 * Handles broadcasting a message to all other users connected to the TCP
 * server.
 * @param conn Connection representing client's TCP connection with the server.
 * @param message_content The text or message content that we're sending to all
 * other users.
 * @note The user input for this request is gathered in the caller, as a
 * stylistic choice. For example the caller can just type /world <message> to
 * send their message, sending their command and its args in one line.
 */
 ...
 * @param conn Connection representing client's TCP connection with the server.
 * @param recipient_username The username of the recipient user that the message
 * is being sent to.
 * @param message_content The text or message content that we're sending to the
 * recipient user.
 *
 * @note User input for this is gathered in the caller. This is merely a
 * stylistic choice.
 */
static void handle_peer_message(conn_t &conn, std::string &recipient_username,
...
// These handle processing responses from the server e.g. response to
...
static void handle_registration_response(message_t &response) {
  ...
  LOG_INFO("[Registration]: Successful, <User id=%d, username=%s>\n",
...
static void handle_broadcast_response(conn_t &conn, message_t &response) {
  ...
  case TAG_P2P_BROADCAST: {
    ...
    print_chat_message(broadcast.sender_username, broadcast.recipient_username,
  ...
  case TAG_WORLD_BROADCAST: {
    ...
    print_chat_message(broadcast.sender_username, "",
                       broadcast.message_content);
    break;
  }
  default: {
 ...
 * @param conn Connection representing client's TCP connection with the server.
 * @return true on successful reading/processing, false when the servver stops
 * responding
 */
static bool handle_server_response(conn_t &conn) {
  // General Workflow
  // 1. Read one and parse one full message into response
  // 2. Based on human-readable response message handle the response message
  // with one of our handlers
  message_t response{};
...
/**
 * Handler function for the background thread that listens for asynchronous
...
static void socket_thread_handler(conn_t &conn) {
  ...
  // 1. Server closes connection: Sends a FIN packet, which should trigger the
  // POLL readiness API, then the handle_broadcast_notification will return
  // false. Then the user quits on their own.
  // 2. User types '/quit', where main thread signals peer to break out of loop
  // and terminate.
...
static bool command_shell(conn_t &conn, std::string &input_line) {
  ...
  std::string command;
  if (!(ss >> command))
    return true; // Extract first word and lowercase it; this is the command
