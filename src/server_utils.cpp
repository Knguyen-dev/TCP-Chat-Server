 ...
 * @param epollfd File descriptor for the epoll instance that our application is
 * using.
...
// -----------------------------------
/**
 * Adds an unauthenticated client connection to the connection table and epoll
 * instance
...
static void add_connection(int epollfd, int connfd) {
  ...
  // 1. Create nonblocking socket and fd to interest list; reading and
  // edge-triggered
 ...
 *
 * @param request Request message struct that has the data needed to register a
 * user.
 * @param response Empty response message that this function will
 * modify/populate with data based based on the status of the current request.
...
static int register_user(message_t &request, message_t &response) {
  ...
  if (rc != 0) {
    ...
    response = build_server_response(REGISTER, static_cast<response_code_t>(rc),
                                     NULL, 0);
    return rc;
  }
  ...
  // case-insensitive uniqueness. So 'Alice' and 'alice' are considered the same
  // username, and if 'Alice' is registered first, then 'alice' will be rejected
  // as a registration username.
  user_t user{};
 ...
 * @param response Empty response message that the callee (this function) will
 * modify with information.
...
static int login_user(int connfd, message_t &request, message_t &response) {
  ...
  if (rc != 0) {
    response =
        build_server_response(LOGIN, static_cast<response_code_t>(rc), NULL, 0);
    return rc;
  }
  ...
  if (rc != 1) {
    if (rc == 0) {
      LOG_DEBUG("User with username '%s' not found\n",
                credentials.username.data());
      response =
          build_server_response(LOGIN, RESP_ERROR_USER_NOT_FOUND, NULL, 0);
  ...
  if (user.password != credentials.password) {
    ...
    response =
        build_server_response(LOGIN, RESP_ERROR_INVALID_CREDENTIALS, NULL, 0);
 ...
 * @param response Response message that the callee will populate with data
 * about the result of this operation.
 * @return 0 on success, non-zero return code otherwise.
 */
static int handle_broadcast_message(int connfd, int epollfd, message_t &request,
                                    message_t &response) {
  ...
  case TAG_WORLD_BROADCAST: {
    ...
    if (ret != 0) {
      ...
      response = build_server_response(CHAT, static_cast<response_code_t>(ret),
                                       NULL, 0);
      return ret;
    }
    ...
    // If the sender username in the broadcast doesn't match the actual TCP
    // client
    // TODO:
    // In reality, the server should autofill the username server side rather
    ...
    // NOTE: Skip the sender, because after this function returns, we'll add
    // this response to the sender's outgoing buffer. If you don't skip the
    // sender you're going to send the world-message twice.
    std::vector<uint8_t> serialized_response;
    write_message_to_buffer(serialized_response, response);
    for (size_t i{0}; i < conn_manager.get_size(); i++) {
      // a. Recipient is only valid if it's authenticated AND it isn't the
      // sender
      bool is_valid_recipient =
          has_flag(conn_manager.flags[i], ConnFlags::IS_AUTH) &
          (i != static_cast<size_t>(connfd));
  ...
  case TAG_P2P_BROADCAST: {
    ...
    // NOTE: The response message sent to the peer will also act as the response
    // message to the client's P2P broadcast request.
    ...
    if (recipient_conn_fd == -1) {
      response =
          build_server_response(CHAT, RESP_ERROR_USER_NOT_FOUND, NULL, 0);
      return RESP_ERROR_USER_NOT_FOUND;
    }
...
static bool try_one_request(int connfd, int epollfd) {
  ...
  // NOTE: At this point you know that message_t::payload points somewhere in
  // conn_t::incoming due to parse_message handling the heavy lifting.
  ...
  default:
    ...
    response = build_server_response(request.type, RESP_ERROR_UNKNOWN_COMMAND,
                                     NULL, 0);
    break;
  }
...
void handle_read_connection(int connfd, int epollfd) {
  ...
  // 4. Erase bytes from the start of conn_t::outgoing in
  // handle_write_connection NOTE: while loop means if we read multiple
  // requests, then we'll try to process multiple (request pipelining).
...
void accept_all_connections(int listenfd, int epollfd) {
  ...
  while (true) {
    ...
    int conn_fd =
        accept(listenfd, (struct sockaddr *)&client_address, &client_len);
    if (conn_fd < 0) {
...
int open_listenfd(char *port) {
  ...
  - ai_flags:
    a. AI_PASSIVE: Configured to let it know we want addresses that can be used
  by servers (passive/listening sockets). Equivalent to INADDR_ANY as well. b.
  AI_ADDRCONFIG: Returns IPv4 or IPv6 addresses, depending on what the host is
  configured to use. c. AI_NUMERICSERV: Indicates that 'service' parameter will
  be a port number.
  */
  ...
  1. Iterate through linked list of addrinfo structs to find an address to bind
  to a. Attempt to create a listening socket for the following protocol (IPv4,
  IPv6, etc.) socket type (connection-based or datagram based), and transport
  layer protocol. b. If listenfd < 0, we failed to create a socket for that
  addrinfo struct. Not a critical error, so attempt the next addrinfo struct. c.
  Configure the socket s.t. we can reuse it immediately after closing it. Good
  for fast restarts. d. Attempt to bind the socket to an IP:port, if it works,
  then break out of loop since we got a success. e. Otherwise, we failed to bind
  the socket, attempt to close the socket to regain resources. If that also
  fails, exit out of function.
  2. Free address info linked list since we're done with it.
  3. If p is undefined, then we didn't even iterate through the list. This
  indicates the OS didn't find any suitable addresses for the configurations
  applied.
  4. At this point we have a suitable listening socket. Attempt to open the
  listening socket which will cause our server to start buffering incoming
  connections.
  */
  ...
  if (listen(listenfd, LISTENQ) < 0) {
    LOG_ERROR(
        "open_listenfd listen error: Error opening the listening socket!\n");
    freeaddrinfo(listp);
    close(listenfd);
    return -1;
  }

  // Get the raw IP address (instead of hostname) and give port number as a
  // string
  char host[1025];
  char service[32];
  int s = getnameinfo(p->ai_addr, p->ai_addrlen, host, sizeof(host), service,
