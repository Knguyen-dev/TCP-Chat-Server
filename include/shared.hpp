...
/**
 * Performs a bitwise negation on the connection flag and returns the resulting
 * flags.
 */
...
struct conn_t {
  std::vector<uint8_t>
      incoming; // Buffer stores data read (and needs to be parsed) by our
                // user-space app from the kernel.
  std::vector<uint8_t>
      outgoing;  // Buffer stores  data that needs to be written to the peer.
  user_t user{}; // Pointer to the user associated with the connection; if
                 // pointer isn't NULL, this is an authenticated user
  int fd = -1; // Integer containing the file descriptor for the TCP connection
               // socket.
 ...
 * - Using fgets, if the user types more than the buffer, extra characters will
 * stay in the stdin buffer waiting for our next fgets() call. This function
 * will clear those extra characters automatically (buffer flushing).
 ...
 * @param buffer Reference to string buffer that we're going to write input
 * into.
 */
