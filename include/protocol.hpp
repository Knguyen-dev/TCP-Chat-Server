...
typedef struct {
  ...
  uint8_t rc; // Response code, assuming type = ACK (the message represents a
              // server response)
 ...
 *
 * @param buf A double pointer to a buffer. The motivation is that with a single
 * pointer *buf modifications like buf += 1, doesn't update the pointer in the
 * caller. However by using a double pointer, we'll be able to update the buffer
 * pointer in the caller.
 * @param tag The tag that we're giving the TLV; "what is it?".
 * @param len The size of the data (in bytes from 0-255) of the value, "how many
 * bytes is it?".
 * @param value A pointer to the value that we want to write into the buffer
 * @param convert_to_network 1 to convert the value into network-byte-order,
 * otherwise 0 for no conversion.
 *
 * @note
 ...
 *
 * - Limitations: Since len is a uint8_t we can only represent payloads of size
 * [0, 255] bytes. If we wanted to represent bigger payloads, we'd simply
 * upgrade to uint16_t, allowing us to write values of size [0, 65335] bytes,
 * which will later be useful for messages. Of course, if we decide to use
 * uint16_t, we'd need to ensure the 16-bit integer is represented in
 * network-byte-order and probably use memcpy to copy from bytes from the
 ...
 * @param data Pointer to the a buffer we'll read data from and copy into said
 * buffer.
 ...
 * @param buffer Read-only buffer of data that contains the message header. This
 * represents the conn_t::incoming that contains the message we got from the
 * remote.
 * @param message Reference to the message struct that we're going to populate
 * with parsed header information.
 *
 * NOTE: This can be used client and server-side. On the server side, we assume
 * the buffer has the first `MSG_HEADER_SIZE + 1` bytes of a message. For now,
 * we want that extra byte so that we can get a pointer to the start of the
 * message body. The idea being that we don't have to manually copy the memory
 * from the message body into a message_t struct, just use a pointer to the
 * conn_t::incoming, which will retain that message body we're done parsing the
 * message.
 */
 ...
 * @param buffer Buffer that we're appending data to the end to. This represents
 * the conn_t::outgoing buffer
 ...
 * have everything set already, we only need to deal with writing bytes,
 * endian-ness etc. This operation appends the bytes to the end of the buffer.
 * This operation should ont be interrupted.
 */
 ...
 * @param message Message struct that'll be populated which'll contain readable
 * information about the processed message.
 * @return 0 on success, otherwise -1.
 */
int read_one_message(int fd, message_t &message);

/**
 * Performs a blocking write to write one full message from the conn_t::outgoing
 * into the socket
 * @param fd Fd for the tcp connection socket
 ...
 * @param request Request message that we're going to populate with user
 * registration data.
 * @param credentials Struct containing the user registration credentials that
 * the user inputted in.
 * @param message_len Length of the message, which will be populated/updated
...
/**
 * Parses a user registration request message and populates credentials object
 * with it.
 *
 * @param msg User registration request message sent by the client that needs to
 * be parsed.
 ...
 * @param response Response message representing the server's response to a
 * registration request.
 * @param user Empty user that will be populated with the response's payload
 * data.
 ...
 * @param credentials Credentials object that we'll populate after parsing the
 ...
 * @param user User account that was successfully logged in, which will be
...
/**
 * Parses a successful login response message from the server. It processes the
 * payload.
 *
 * @param response Response object that represents a successful login response
 * message sent by the server.
 * @param user Empty user struct that will be populated by the callee with the
 * info of the user that was just logged in with.
 ...
 * @param request_buffer Empty buffer that'll be populated with the serialized
 * data for the world broadcast request.
 * @param broadcast World broadcast whose data we want to put into the request
 * message.
 * @param message_len Reference that'll be populated with the length of the
 * entire message.
 * @return 0 on success, -1 otherwise.
 ...
 *
 * @param request Request message sent by the client that indicates they're
 * doing a world broadcast.
 * @param broadcast World broadcast that we'll fill with data from this request
 * message.
 ...
 * @param response The response message that we'll fill and write to all other
 * users.
 * @param broadcast the broadcast object that contains the message that they
 * want send to all other users.
 * @note This is used by the server. So client sends world-broadcast and now the
 * server has to actually send that broadcast to all other users. Here we build
 * the response message that's sent to all other users. Same as
 * build_world_broadcast_request, meaning the message format is same for client
 * and server.
 */
 ...
 * - Since world broadcasts and world broadcast notifications are the same, the
 * logic for parsing them are the same.
 */
 ...
 * @param request_buffer Empty buffer that'll be populated with the serialized
 * request data.
 * @param broadcast P2P Broadcast object that'll be serialized and stored in the
 * request buffer.
 * @param message_len Reference that'll be populated with the length of the
 * entire message.
 * @return 0 on success, -1 on error.
 * @note This is supposed to be used by the client since they're the one
 * initiating the p2p broadcast request.
 */
 ...
 * @param broadcast Broadcast message that we'll populate with data from the
 * request message
 * @note Supposed to be used by the server.
 */
 ...
 * @param response Response message we're populating data with, which we'll send
 * to the recipient of the P2P response.
 * @param broadcast P2P broadcast sent by the client.
 * @return 0 on success, response code otherwise.
 * @note This is supposed to be used by the server. The client sends a P2P
 * broadcast request, and then we build a message that we'll send to the
 * recipient. This message will contain the message from the sender. Identical
 * format to creating a P2P broadcast, so client and sender have the same
 * message format.
 */
...
/**
 * Given a request message for a chat, return the type of broadcast it wants to
 * do.
 *
 * @param request The request message sent by the client, where message->type =
 * CHAT.
 ...
 * @param payload Pointer to start of the buffer that contains the payload data.
 * This is assumed to already have the stream of TLVs already contained in it.
 * @param payload_len Length of the payload buffer
 * @return Response message that represents the server response.
 *
 * @note For this protocol, the response can reuse the request message type.
 * That means a registration response can be type REGISTER, a login response can
 * be type LOGIN, and so on. This avoids a separate ACK-only type while
 * preserving the response code.
 */
